...
/**
 * get_section_type32 - gets symbol type based on section
 * @symbol: the symbol to check
...
/**
 * get_symbol_type32 - determines the symbol type character
 * @symbol: the symbol to check
 * @section_headers: pointer to section headers array
 * Return: the symbol type character
 */
char get_symbol_type32(Elf32_Sym symbol, Elf32_Shdr *section_headers)
{
	...
	if (symbol.st_shndx < SHN_LORESERVE)
		return (get_section_type32(symbol, section_headers));
	return ('?');
}
